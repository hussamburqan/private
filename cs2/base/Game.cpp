...
bool CGame::InitAddress()
{
	...
	this->Address.ClientDLL = GetProcessModuleHandle(ProcessMgr.HANDLE, ProcessMgr.ProcessID, XS("client.dll"));
	this->Address.MatchDLL = GetProcessModuleHandle(ProcessMgr.HANDLE, ProcessMgr.ProcessID, XS("matchmaking.dll"));
...
{
	std::string dtbName = XS("dtb.txt");
	if (strcmp(uszName, dtbName.c_str()) == 0)
		cbSize = cb;
}
...

{
	...
	std::string vfsPath = XS("\\misc\\procinfo\\");
	result = VMMDLL_VfsListU(HANDLE, (LPSTR)vfsPath.c_str(), &VfsFileList);
	if (!result)
		return false;
	...
	std::cout << XS("[-] Failed to patch module\n") << std::endl;
	return false;
}
